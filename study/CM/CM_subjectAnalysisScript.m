...
doAntomy = 0;
doPSIR = 0;
doGLMbc = 0; % fit boxcar GLM to both sparse and continuous dat
doMakeFlatmaps = 0; % create flat maps
doMakeARrois = 0; % define auditory responsive base on F-test (only sparse?)
doDeconv = 0; % fit deconvolution model to continuous data
doHRF = 0; % calculate average max HRF across auditory responsive voxel
doGLMdg = 0; % fit double-gamma to estimated HRF and use parameter in third-step GLM (for both sparse and continuous data)
doConvertOverlays = 0; % convert overlays to ERBs (?)
doDeleteOverlays = 0; % compute gradient reversals from GLM estimates
doConvertvol2FlatAvDepth_GLM = 0; % export flat GLM data to volume format
doGradientReversal_GLM = 0; % compute gradient reversals from GLM estimates
doROIsGLM = 0; % manually create two gradient ROIs (for pRF analysis)
doGetDATA_GLM = 0;  % get PF and TW estimates from pRF using GLM gradient ROIs
doROIRestrict_GLM = 0; % union of gradient ROIs + expanded (+restrict to cortical surface?)
dopRF = 0; % run pRF analysis
doConvertvol2FlatAvDepth_pRF = 0; % export pRF flat data to volume format
doGradientReversal_pRF = 0; % compute gradient reversals from pRF estimates
doROIspRF = 0; % manually create two gradient ROIs ( NOT USED)
doGetAndRestrictDATA_pRF = 0; % get PF and TW estimates from pRF using GLM gradient ROIs
doGRrois = 0; % draw reversal ROIs
doCorticalMagnification = 0;
doPrintFigures = 1; % print flat maps
doPrintFigureSpotlight = 1; % sets coordinates of a mask to only show a restricted spotlight when printing figures

%% define subjects
% iSubs2Run = [1,2,3,4,5,6,7,8];
iSubs2Run = [1,3,4,5,6,7,8];
...
for iSub = 1:length(iSubs2Run)
    ...
    if doMakeARrois
        ...
        % Project through depths 0 to 1 to remove voxels outside of grey matter for ALL ROIs
...
%                 eval(['data.' ROInames{iROI} ' = struct;']);
    ...
    if dopRF
        ...
        [thisView, pRFParams] = script_pRFAnalysis(thisView,pRFInfo,glmInfo,pRFInfo.pRFrestrictROI,1,0);
    end
    ...
    if doGradientReversal_pRF
        ...
        thisView = script_flatMapAnalysis(thisView,Info,subjectInfo,Info.gradReversalInfo.groupBase,pRFInfo.pRFanalysisName,pRFInfo.pRFgradientReversalOverlay,'[18 18 21]');
    end
    ...
    %         analName = {'GLM', 'pRF'};
    roiAnalName = {'GLM'};
    roiAnalNum = 1; % a bit of a hack to account for no pRF rois
    if doGRrois
        for iSide = 1:length(Info.Sides)
            ...
            disp([Info.Sides{iSide}, 'Higha_GLM'])
            disp([Info.Sides{iSide}, 'Low_GLM'])
            disp([Info.Sides{iSide}, 'Highp_GLM'])
            disp('hit F5 when done')
            ...
            for iAnal = 1:length(roiAnalName)
                for iAP = 1:length(AP)
                    clear rois
                    
                    % define ROI names then get ROI numbers then put in a list
                    HighROIname = [Info.Sides{iSide}, 'High' AP{iAP} '_' roiAnalName{iAnal}];
                    roiList(1) = viewGet(thisView,'roinum',HighROIname);
                    
                    LowROIname = [Info.Sides{iSide}, 'Low', '_' roiAnalName{iAnal}];
                    roiList(2) = viewGet(thisView,'roinum',LowROIname);
                    
                    GRROIname = [Info.Sides{iSide}, 'GR' AP{iAP} '_' roiAnalName{iAnal}];
                    ...
                    HighROIname = [Info.Sides{iSide}, 'High' AP{iAP} '_' roiAnalName{iAnal} '_ex'];
                    roiList(1) = viewGet(thisView,'roinum',HighROIname);
                    
                    LowROIname = [Info.Sides{iSide}, 'Low', '_' roiAnalName{iAnal} '_ex'];
                    roiList(2) = viewGet(thisView,'roinum',LowROIname);
                    
                    GRROIname = [Info.Sides{iSide}, 'GR' AP{iAP} '_' roiAnalName{iAnal} '_ex'];
                    roiList(3) = viewGet(thisView,'roinum',GRROIname);
            ...
            for iGroup = 1:length(glmInfo.groupNames) % change change this if we want to calculate for both groups or not
                ...
                for iAnal = 1:length(analysisNames)
                    ...
                    for iAP = 1:length(AP)
                        ...
                        clear roiList
                        HighROIname = [Info.Sides{iSide}, 'High' AP{iAP} '_' roiAnalName{roiAnalNum} '_ex_Vol'];
                        roiList(1) = viewGet(thisView,'roinum',HighROIname);
                        
                        LowROIname = [Info.Sides{iSide}, 'Low', '_' roiAnalName{roiAnalNum} '_ex_Vol'];
                        roiList(2) = viewGet(thisView,'roinum',LowROIname);
                        
                        GRROIname = [Info.Sides{iSide}, 'GR' AP{iAP} '_' roiAnalName{roiAnalNum} '_ex_Vol'];
                        roiList(3) = viewGet(thisView,'roinum',GRROIname);
